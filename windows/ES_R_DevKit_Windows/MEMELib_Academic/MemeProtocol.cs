...
public static partial class MemeProtocol
{
    ...
    /// <summary>
    /// AUP_REPORT_ACADEMIA1(0x98) を 1 サンプルへ展開する。
    /// EOG は 1 パケットに 2 組(添字 1 / 2)入る。
    /// </summary>
    public static AcademicStandardData ParseStandardData(ReadOnlySpan<byte> p)
    {
        if (!IsValidPacket(p))
        {
            throw new ArgumentException("not a 20-byte MEME packet", nameof(p));
        ...
        var data = new AcademicStandardData
        {
            Cnt = ((p[3] << 8) & 0x0F00) | p[2],
            BattLv = p[3] >> 4,
            AccX = ReadInt16(p, 4),
            AccY = ReadInt16(p, 6),
            AccZ = ReadInt16(p, 8),
            EogL1 = ReadInt16(p, 10),
            EogR1 = ReadInt16(p, 12),
            EogL2 = ReadInt16(p, 14),
            EogR2 = ReadInt16(p, 16),
        };

        data.EogH1 = unchecked((short)(data.EogL1 - data.EogR1));
        data.EogH2 = unchecked((short)(data.EogL2 - data.EogR2));
        data.EogV1 = unchecked((short)(0 - ((data.EogL1 + data.EogR1) / 2)));
        data.EogV2 = unchecked((short)(0 - ((data.EogL2 + data.EogR2) / 2)));
        return data;
    }

    /// <summary>AUP_REPORT_ACADEMIA3(0x9A) を 1 サンプルへ展開する。値は LE の符号付き 32bit。</summary>
    public static AcademicQuaternionData ParseQuaternionData(ReadOnlySpan<byte> p)
    {
        if (!IsValidPacket(p))
        {
            throw new ArgumentException("not a 20-byte MEME packet", nameof(p));
        ...
        return new AcademicQuaternionData
        {
            Cnt = ((p[3] << 8) & 0x0F00) | p[2],
            BattLv = p[3] >> 4,
            QuaternionW = ReadInt32(p, 4),
            QuaternionX = ReadInt32(p, 8),
            QuaternionY = ReadInt32(p, 12),
            QuaternionZ = ReadInt32(p, 16),
        };
    }
    ...
    private static int ReadInt32(ReadOnlySpan<byte> p, int offset) =>
        BinaryPrimitives.ReadInt32LittleEndian(p.Slice(offset, 4));
}
